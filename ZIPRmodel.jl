...
# ╔═╡ 70bf6ead-b755-431c-ab21-2389da5c7490
begin
	using PlutoUI
	#TableOfContents()
end
...
md"""
# Refinery Downtime Prediction Model

Using empirical downtime data from Gulf Coast refinery disruptions following hurricanes over the past 20 years.
"""

# ╔═╡ 809fbe11-f8a7-427d-a620-7044ec877e3b
md"""
## Part I: zero-inflated Poisson regression

This model defines a variable $z_i$ to be 1 if a refinery experienced downtime and 0 if it did not.
This variable is estimated using logistic regression:
```math
\begin{align}
z_i &\sim \mathrm{Bernoulli}(\theta_i) \\
\mathrm{logistic}(\theta_i) &= -\tau X_i \beta
\end{align}
```
where $X_i$ is assumed to include a column of ones (so $\beta_1$ is the intercept).

Given $x_i$, the days of downtime that a particular refinery experienced, $y_i$, follows a zero-inflated Poisson regression model:
```math
y_i \sim \begin{cases} \mathrm{Poisson}(\lambda_i) & z_i = 1 \\ 0 & z_i = 0 \end{cases}
```
where $\lambda_i$ is the parameter of the [Poisson distribution](https://juliastats.org/Distributions.jl/stable/univariate/#Distributions.Poisson), of which the natural parameterization is:
```math
\begin{align}
\mathrm{log}(\lambda_i) &= \beta X_i \\
\end{align}
```
so the parameters to estimate are just $\beta$ and $\tau$.
"""

# ╔═╡ 341d21b1-1ce0-47ed-9cf8-0683877a2f95
let
	p = plot(title="Poisson Distributions", legend=:topright)
	for λ in (1, 4, 10, 20)
		label = L"$\mathrm{Poisson}(%$λ)$"
		plot!(p, Poisson(λ), label=label, markershape=:circle)
	end
	p
end

# ╔═╡ fc97ae08-a8ab-4e43-803f-1d5b7516ead7
plot(logistic, label=false, title="Inverse Logit Function")
...
# ╔═╡ 0576ebf8-0e04-4e55-8a85-ad63cb276339
function load_data()
	fname = "C:/Users/kcapp/OneDrive/Documents/grad school/research/Refinery Downtime Prediction Model/gcfdowntime_dataset.csv"
	return DataFrame(CSV.File(fname))
end;

# ╔═╡ f1984632-c90b-4e14-b119-089f52fe9f9e
data = load_data()

# ╔═╡ c21c23d4-016f-45e6-ad2e-1cf771448838
function format_poisson_data(M)
	
	X = hcat(ones(length(M.y)), M.x1, M.x2, M.x3, M.x4, M.x5, M.x6, M.x7, M.x8, M.x9, M.x10, M.x11, M.x12, M.x13, M.x14, M.x15, M.x16, M.x17, M.x18, M.x19, M.x20)
	y = M.y
	z = y .!== 0
...
# ╔═╡ c5ee1d1d-0782-4acf-a6ce-b245eef544e2
begin
	X, y, z = format_poisson_data(data)
end;

# ╔═╡ 8d7c681a-ab15-4662-9c21-9bf574545195
md"""
### Task 2: visualize data

Series of plots including histogram of y and scatter plots of several Xs against y.

"""

# ╔═╡ 94c1d908-1956-4e4a-b192-2e436e5163ce
histogram(y, label="Days of Downtime", xlabel="Days", ylabel="Count", title="Gulf Coast Storm-Induced Refinery Downtime")

# ╔═╡ 2eff8cc8-7c23-4f56-9988-4523769e60bd
scatter(X[:,2],y, label=false, xlabel="Facility Flood Extent (%)", ylabel="Days of Downtime")

# ╔═╡ 1fc21eb8-8a8b-4597-baac-9bb2415d0702
scatter(X[:,3],y, label=false, xlabel="Peak Facility Flood Depth (ft)", ylabel="Days of Downtime")

# ╔═╡ fbe3d8ea-0e34-4fc5-a923-78042dc072a9
scatter(X[:,4],y, label=false, xlabel="Peak Wind Velocity at Facility (mph)", ylabel="Days of Downtime")

# ╔═╡ 5b536429-4cd0-46ab-a80c-208d271bcc77
scatter(X[:,6],y, label=false, xlabel="Facility Population", ylabel="Days of Downtime")

# ╔═╡ 320e5242-d17d-45e7-897d-b3c9f104169c
scatter(X[:,7],y, label=false, xlabel="Facility Age", ylabel="Days of Downtime")

# ╔═╡ 20c1e0db-11ab-442e-aab0-240fd1ecf592
scatter(X[:,8],y, label=false, xlabel="Facility Complexity Index", ylabel="Days of Downtime")

# ╔═╡ e97f6603-486f-4c85-9d11-0924d8fda22a
scatter(X[:,9],y, label=false, xlabel="Facility Refining Capacity", ylabel="Days of Downtime")

# ╔═╡ 89cb6bd4-c423-4ac8-b7f4-12c2d15ea0d2
scatter(X[:,18],y, label=false, xlabel="Facility Hydrogen Production Capacity", ylabel="Days of Downtime")

# ╔═╡ 8694c60f-97cf-41fe-8c39-8fc3587ce9e9
scatter(X[:,11],y, label=false, xlabel="Facility Coking Capacity", ylabel="Days of Downtime")

# ╔═╡ 27c9d81f-51a9-4eb6-bae7-4541017a48f1
scatter(X[:,12],y, label=false, xlabel="Facility Catalytic Cracking Capacity", ylabel="Days of Downtime")

# ╔═╡ fea45db9-2d4d-437a-bdbc-2bc537fe1347
scatter(X[:,21],y, label=false, xlabel="Facility Acreage", ylabel="Days of Downtime")

# ╔═╡ d774747c-11a7-4c67-aa6a-a855622890fb
md"""
### Task 3: fit a model

Next, we write a model in Turing to do inference on this zero-inflated Poisson model.

"""

# ╔═╡ e556884c-c1d0-4929-89f0-a807457acd9f
@model function zero_inflated_poisson(X, y, z)
	
	N, P = size(X)

	# priors
	β ~ filldist(Normal(0, 0.5), P)
	τ ~ Normal(0, 0.5)
...
# ╔═╡ 5d2aac7a-bb05-42ab-bc2a-033cb0c24d8e
sample(zero_inflated_poisson(X, y, z), Prior(), 10000);

# ╔═╡ 9d677b3a-2e47-4b3f-8fda-d61e0a37711c
begin
...
# ╔═╡ 370f25fc-4c5f-43fb-bbf4-05787961273b
zip_model = zero_inflated_poisson(X, y, z);

# ╔═╡ 35da3bc2-e16e-4cae-8992-251557a58da2
mel_estimate = optimize(zip_model, MLE())

# ╔═╡ eaa9d109-e5c9-4299-a8d2-65340e865ea2
md"""
### Task 4: identify parameters
"""

# ╔═╡ 571f048b-a583-4c63-80f5-51f56f730adf
poisson_chains = let
	model = zero_inflated_poisson(X, y, z)
...
# ╔═╡ 95c9bdb8-7523-403d-9e1e-6400450f8a4a
plot(poisson_chains)

# ╔═╡ afaf15af-e83f-4131-a3fa-b2ec9c58de99
summarystats(poisson_chains)

# ╔═╡ ebc8d5f0-63f8-4be8-8fa2-2d4fa2b04821
β, τ
...
git-tree-sha1 = "689c9fd59de10ff5bae70a151616fd4a891c4234"
