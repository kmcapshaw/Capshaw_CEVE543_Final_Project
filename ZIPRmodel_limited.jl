...
\mathrm{logistic}(\theta_i) &= \beta X_i
\end{align}
```
...
```math
\begin{align}
\mathrm{log}(\lambda_i) &= \alpha + \beta X_i \\
\end{align}
```
so the parameters to estimate are just $\beta$ and $\alpha$.
"""
...
function load_data()
	fname = "C:/Users/kcapp/OneDrive/Documents/grad school/research/Refinery Downtime Prediction Model/data/gcfdowntime_dataset_limited.csv"
...
function format_poisson_data(M)
	...
	X = hcat(ones(length(M.y)), M.x1, M.x2, M.x3, M.x4, M.x5, M.x6, M.x7, M.x8, M.x9)
	y = M.y
	z = y .!== 0
...
# ╔═╡ 39b3f9f7-3e34-415f-a6ba-bb155f30b30f
md"""
### Task 5: test predictions
"""

# ╔═╡ d8a1cb44-aa0e-4348-8567-31e765c4316c
function simulate_yᵢ(Xᵢ, β, α)
	# data-related part
	μ = sum(β .* Xᵢ) # need to reverse link function on this
	# simulate z
	θ = logistic(μ) # this is the name of the StatsFuns inverse logit function
	z = rand(Bernoulli(θ))
	# simulate y | z, data
	if z == 0
		y = 0
...
# ╔═╡ 2ed907ec-bb95-4a5d-a7b2-7621338563b6
function predict_zero_inflated_poisson(X, β, α)
	N = size(X,1)
	y = [simulate_yᵢ(X[i, :], β, α) for i in 1:N]
	z = y .!== 0
	return y
end;

# ╔═╡ a9b6d8a0-be20-4963-b84b-45aa889a0a15
begin
	β = [-0.160267 0.725575 0.157353 0.253217 -0.196492 0.0755549 -0.0387398 1.21813 -2.50383 -0.296063]
	α = 0.591753
	test_X = X
	new_y = predict_zero_inflated_poisson(test_X, β, α)
end;
...
# ╔═╡ 35da3bc2-e16e-4cae-8992-251557a58da2
 mle_estimate = optimize(zip_model, MLE())
...
# ╔═╡ f08f3c19-a184-4684-91b9-1d3f762e7db4
rmsd(y, new_y)

# ╔═╡ 75ef0e60-e728-4594-8650-36d5da5b9c48
meanad(y, new_y)

# ╔═╡ edbfbfbc-fd3c-4195-bdad-a03a8f4f0395
resid = y .- new_y

# ╔═╡ 6aafb01f-6a5c-4d75-93fc-7699385fcd00
new_y

# ╔═╡ 4fbdd784-de85-4cb1-b167-8282458c1e7a
let
	scatter(new_y, resid, legend=false, xlabel="Predicted Downtime", ylabel="Residuals")
	hline!([0], linestyle=:dash)
	#savefig("C:\\Users\\kcapp\\OneDrive\\Desktop\\resids.png")
end

# ╔═╡ 1d71b22d-ce23-4aab-9551-48e8cb3da5ae
let
	scatter(new_y, y, legend=false, xlabel="Predicted Downtime", ylabel="Observed Downtime")
	f(x) = x
	plot!(f, 0, 360, linestyle=:dash)
	#savefig("C:\\Users\\kcapp\\OneDrive\\Desktop\\y-ynew.png")
end
