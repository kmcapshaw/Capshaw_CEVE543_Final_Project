...
# ╔═╡ 7644b391-3ae6-4779-ae59-bd447f0061e5
function rand(d::ZIPoisson)
    return rand() <= d.w ? 0 : rand(Poisson(exp(d.logλ)))
end;

# ╔═╡ b67ea921-110b-40ad-8c3c-379234ca2d8d
inv_logit(x) = 1/(1 + exp(-x));

# ╔═╡ d7d513dd-4703-4031-94ca-15c9d3d66895
...
# ╔═╡ b8abca66-f832-4205-abd5-8b71b797b5b2
begin
	rand(d::ZIPoisson, N::Int) = map(_->rand(d), 1:N)
	Random.seed!(74591)
	new_data = rand(ZIPoisson(-1.5, .70), 1000)
end

# ╔═╡ 059f19d2-bea0-4b49-a0ee-c07add04c904
begin
	n_samples = 3000
	n_adapt = 1500
	config = NUTS(n_adapt, .65)
	n_chains = 4
	chain = sample(model(X), config, MCMCThreads(), n_samples, n_chains, progress=true)
end
